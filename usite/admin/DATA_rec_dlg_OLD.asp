<%
	...
	if(!isNaN(id))
	{
		...
		else
		{
			...
			{	
				...
				//Response.Write("form_cond: en = _oSettings.enumsettings["+_oSettings.fields.item(idx).text+"] = "+_oSettings.enumsettings[_oSettings.fields.item(idx).text]+"<br>")
				
				var en = _oSettings.enumsettings[_oSettings.fields.item(idx).text];
				var enl = _oSettings.enumlistsettings[_oSettings.fields.item(idx).text];

				var type = _oSettings.settingdata["FIELD_PROPERTIES"][en+2];
				var format = _oSettings.settingdata["FIELD_PROPERTIES"][en+3];
				...
				else if(_name.substring(0,3)=="ESC")
				{
				...
				else if(_name.substring(0,3)=="CSV")
				{
					...
					var en = enumsettings[_oSettings.fields.item(idx).text];
					var enl = enumlistsettings[_oSettings.fields.item(idx).text];
		...
		for(var i=0;i<fieldID.length;i++)
		{
			...
			var fldesc = oSETTINGS.settingdata["FIELD_PROPERTIES"][en+oSETTINGS.p_idx["columnID"]];
			...
			//if(fields.item(i).text == oSETTINGS.settingdata["FIELD_PROPERTIES"][en+oSETTINGS.p_idx["columnID"]])
				...
				case "img":
					...
					{	
						...
						if(bValidImage)
						{
							...
							//var ext 	= obj.path.substring(source.lastIndexOf("."),source.length).toLowerCase();
	...
	if(bIncludeRichtextLib)
	{
		...
		for(var i=0;i<fieldID.length;i++)
		{
			...
			// TODO transform SETTINGS STRUCTURE
			var s_enum = {"datasetID":(en-1),"columnID":en,"columnName":(en+oSETTINGS.p_idx["columnID"]),"type":(en+2),"format":(en+3)} 
			
			var enl = enumlistsettings[fields.item(i).text];
			var type = oSETTINGS.settingdata["FIELD_PROPERTIES"][s_enum["type"]];
			var format = oSETTINGS.settingdata["FIELD_PROPERTIES"][s_enum["format"]];
			format=(format=="richtext"?DefaultRichtextLib:format);
