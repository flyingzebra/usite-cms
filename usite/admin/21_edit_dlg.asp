...
<%
	...
	var categories = oDB.getrows("select "+catfld.join(",")+" from "+_db_prefix+"reviewtype where rt_typ = "+rev_type+" and rt_dir_lng = \""+_ws+"\"");
	...
	var pages = oDB.getrows("select "+pagfld.join(",")+" from "+_db_prefix+"reviewtype where rt_typ = "+rev_cat_type+" and rt_dir_lng = \""+_ws+"\"");
	...
	if (bSubmitted && bExists==true)
	{
		...
		formarr[enumfld["crea_acc"]] = formarr[enumfld["crea_acc"]] ? formarr[enumfld["crea_acc"]] : Session("uid") 
	}
	else
...
<input type="hidden" name="crea_acc" value="<%=formarr[enumfld["crea_acc"]]%>">
