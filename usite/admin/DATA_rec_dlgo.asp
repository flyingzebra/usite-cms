	...
	if(!isNaN(id))
	{
				...
				for (var objEnum=new Enumerator(FormCollection); !objEnum.atEnd() ; objEnum.moveNext())
				{
					...
					else if(obj.name.substring(0,3)=="CSV")
					{
						...
						if      (type=="multistring" && format.substring(0,3)=="csv")
							...
							rawdata[idx] = (rawdata[idx]?(rawdata[idx]+","):"")+(obj.value?Number(obj.value.replace(/,/,"."))*(format.substring(0,3)=="EUR"?100:1):"");					
						else
		...
		for(var i=0;i<fieldID.length;i++)
		{
			...
			switch(type)
			{
				...
				case "file":
					if (bUpload && Upload.Files.Count==1)
					{
						...
						if(Upload.Files.Count == 1)
							...
							{
								source_name = objEnum.item().ExtractFileName();
								field_name =  objEnum.item().name;
							}
						
						
						var sidx   = field_name?Number(field_name.substring(8,11)):0;
						var filebase = zerofill(dt,10)+"_"+zerofill(id,6)+"_"+zerofill(curfieldID,3)+"_"+sidx;
						var source_path	= Server.Mappath("../images/upload")+"\\";
						var dest			= Server.MapPath(filepath)+ "\\src" + filebase;	
								
						var source = source_path+source_name;
						var sext   = source.substring(source.indexOf("."),source.length);
					...
					if(typeof(value)=="string")
					{
						Response.Write("<input SIZE=1 name=\"FILE"+zerofill(curfieldID,3)+"\" type=FILE value=\""+Server.HTMLEncode(value)+"\"  onchange=main."+name+".value='"+sext+"';main.submit() onmouseover=this.style.cursor='hand' style=\"background-color:white;border:'1px solid #FFFFFF';font:'10px Verdana';text-align=right;\">\r\n");
						Response.Write(Server.HTMLEncode(value))
						Response.Write("<input name="+name+" type=hidden value=\""+(sext?sext:value)+"\">"+(sext?("src"+filebase+sext):value)+"\r\n");
					}
					else
					{
						for(var j=0;j<value.length;j++)
						{
							if(value[j])
							{
								var fldname = "ESC"+zerofill(fieldID[i]+1,3)+"_"+zerofill(j,3);
								Response.Write("<input SIZE=1 name=\"FILE"+zerofill(curfieldID,3)+"_"+zerofill(j,3)+"\" type=FILE value=\""+Server.HTMLEncode(value)+"\"  onchange=main.submit() onmouseover=this.style.cursor='hand' style=\"background-color:white;border:'1px solid #FFFFFF';font:'10px Verdana';text-align=right;\">\r\n");
								Response.Write(Server.HTMLEncode(value[j]))
								Response.Write("<input name="+fldname+" type=hidden value=\""+Server.HTMLEncode(value[j])+"\">\r\n");
							}
						}
						...
						Response.Write("<input SIZE=1 name=\"FILE"+zerofill(curfieldID,3)+"_"+zerofill(j,3)+"\" type=FILE value=\""+Server.HTMLEncode(value)+"\"  onchange=main.submit() onmouseover=this.style.cursor='hand' style=\"background-color:white;border:'1px solid #FFFFFF';font:'10px Verdana';text-align=right;\">\r\n");
						Response.Write(Server.HTMLEncode(value[j]))
						Response.Write("<input name=ESC"+zerofill(fieldID[i]+1,3)+"_"+zerofill(j,3)+" type=hidden value=\""+Server.HTMLEncode(value[j])+"\">\r\n")	
					}
				...
				case "multiref":
					...
					//value = typeof(rawdata[i])!="number" && typeof(rawdata[i])!="string"?rawdata[i].split(","):new Array(new Number(rawdata[i]));
						...
						if(args[0]=="combo")
						{
							...
							if(bArray)
							{
								for(var k=0;k<(value.length+1);k++)
								{
									...
									Response.Write("\r\n<select name="+(bArray?("ESC"+zerofill(fieldID[i]+1,3)+"_"+zerofill(k,3)):name)+" "+attr+">\r\n");
									
									if(!arg["xdef"])
										Response.Write("\t<option value=\"\">\r\n");
									
									for(var j=0;j<arr.length;j+=2)
										Response.Write("\t<option value=\""+arr[j]+"\""+((value[k]?arr[j]==value[k] || Number(arg["xdef"])==(j/2):arr[j]==sessionchoice)?" SELECTED":"")+">"+(arr[j+1]?arr[j+1]:("["+arr[j]+"]"))+"\r\n");
							...
							{
								Response.Write("<select name="+name+" "+attr+">\r\n");
								...
								for(var j=0;j<arr.length;j+=2)
								{
									var rs = "\t<option value=\""
											+arr[j]+"\""
									rs += ((value[k] && value[k].toString()?arr[j]==(bArray?value[0]:value):(Number(arg["xdef"])==(j/2)))?" SELECTED":"")+">"
									rs += (arr[j+1]?arr[j+1]:("["+arr[j]+"]"))+"\r\n"
									Response.Write(rs);
								}
						...
						else if(args[0]=="tree")
						{
							...
							if(bArray)
								for(var k=0;k<value.length;k++)
									Response.Write( oTREE.combobox("name="+(bArray?("ESC"+zerofill(fieldID[i]+1,3)+"_"+zerofill(k,3)):name)+" size=1",value[k]).replace(/ value=0/,"") + "\r\n\r\n" );	
...
// FUNCTIONS
	...
	function fieldheader(curfieldID,fldesc,name,en,enl)
	{
		...
		if(settingspage)
		{
			...
			_fimglink += "<a href="+zerofill(settingspage,2)+"_"+(en || enl?"rec":"cfg")+"_dlg.asp?dt="+(_fdt?_fdt.toString().encrypt("nicnac"):"")+"&ds="+Request.QueryString("dt").Item+"&id="+(_fid?_fid.toString().encrypt("nicnac"):"")+(enl?"":"&i="+curfieldID+"&s=FIELD_PROPERTIES")+" target=_blank>"
			_fimglink += "<img src=../images/"+_fimg+" onmouseover=this.src='../images/"+_faltimg+"' onmouseout=this.src='../images/"+_fimg+"' border=0>"
			_fimglink += "</a>";
		}
