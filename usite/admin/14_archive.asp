...
<%
	...
	var tablefld = new Array("rev_id","rev_title","rev_desc","rev_rev","rev_rt_typ","rev_rt_cat","rev_finish","rev_code","rev_publisher","rev_phone","rev_fax","rev_email","rev_url","rev_crea_acc_id","rev_mod_acc_id","rev_pub","rev_date_published","rev_date_published = '0000-00-00 00:00:00' AS isDatePublised"
	...
	var checktitles = ["Online","Apprear in summary","Locked","Deleted"];
	...
	var lSQL = "select count(*) from "+_db_prefix+"review where rev_rt_typ = "+rev_type+" and (rev_pub & 8) = 8 and (rev_dir_lng = \""+_dir+"\" or rev_dir_lng = \""+_ws+"\")";
...
//Response.Write(lSQL+"<br><br>")	
	...
	var sSQL = "select "+tablefld.join(",")+" from "+_db_prefix+"review where (rev_pub & 8) = 8 and (rev_dir_lng = \""+_dir+"\" or rev_dir_lng = \""+_ws+"\") order by rev_id desc "+limit;
...
<tr>
<td style=background-color:black;text-align:center;font-size:8px;color:gold;font-weight:bold title='Delete'></td>
<%
	for (var i=0;i<checkarr.length;i++)
...
<td style=background-color:black;text-align:center;font-size:8px;color:gold;font-weight:bold title='Document type'>DT</td>
<td style=background-color:black;text-align:center;font-size:8px;color:gold;font-weight:bold title='Document number'>DN</td>
...
<%
	for( var i=0 ; i < overview.length ; i+= tablefld.length )
	{
		...
		if ((overview[i+enumfld["rev_pub"]] & 4) == 0)
			Response.Write("<td><a target=_blank href="+zerofill(rev_type,2)+"_edit_dlg.asp?id="+overview[i+enumfld["rev_id"]].toString().encrypt("nicnac")+"><img src=../images/i_edit.gif border=0></td>");
		else
		...
		Response.Write("<td style='"+rowcolor+"'>"+overview[i+enumfld["rev_rt_typ"]]+"</td>"); // Document type
		Response.Write("<td style='"+rowcolor+"'>"+overview[i+enumfld["rev_rt_cat"]]+"</td>"); // Document number
