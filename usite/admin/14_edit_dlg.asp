...
<%
	...
	try { var Upload = Server.CreateObject("Persits.Upload.1");  var FormCollection = Upload.Form; bUpload = true; bMultipart = true} catch(e) { bMultipart=false; Response.Write("Persits.Upload.1 failed") }
	...
	// OVERRIDE !!
	Response.Write("bMultipart = false")
	bMultipart = false;
...
*/
	...
	*/
	...
	if (bUpload)
	{
		...
		var dbnamearr   = ["rev_img1","rev_img2","rev_img3","rev_img4","rev_img5","rev_img6","rev_img7"];
		var VARsizearr	= ["100x100" ,"220x100" ,"100xVAR" ,"150xVAR" ,"220xVAR" ,"500xVAR","VARxVAR"];
...
<tr>
	<td align=right style=background-color:white>title »</td><td align=right style=background-color:#e0e0e0><INPUT name=title type=text value="<%=formarr[enumfld["title"]]%>" size=<%=ctxtwidth%>></td>
	<td align=left style=background-color:#e0e0e0 rowspan=3 style=text-align:center>
			<table>
			<tr>
					<td valign=bottom><table height=100 width=100 bgcolor=black cellspacing=1><tr><td style=font-size:12px;background-color:white align=center <%if (formarr[enumfld["actimg"]] & 1) {%> background=../images/articles/<%="img"+zerofill(id,10)+"_1.jpg?"+(Math.floor(Math.random()*10000)) %> <%}%>><p align=left><%=oButton.get("main.actimg.value=main.actimg.value&(1^0xff);main.submit();","../../images/exit.gif","../../images/exit.gif","remove image")%></p>JPG<br>Image<br><br><input TYPE=FILE SIZE=1 NAME=FILE1 onchange=main.submit() onmouseover=this.style.cursor='hand' style="background-color:white;border:'1px solid #FFFFFF';font:'10px Verdana';text-align=right;">&nbsp;&nbsp;&nbsp;</td></tr></table></td>
					<td valign=bottom>
						<table>
							...
							<tr><td><%=resizearr?resizearr[6]:""%><br><input TYPE=FILE SIZE=1 NAME=FILE7 onchange=main.submit() onmouseover=this.style.cursor='hand' style="background-color:white;border:'1px solid #FFFFFF';font:'10px Verdana';text-align=right;"></td></tr>							
						</table>
					</td>
			</tr>
			</table>
	</td>
	<td align=left style=background-color:white rowspan=3><table cellspacing=0 cellpadding=0 width=100% border=0><tr><td height=150 style=background-color:white><br><br>« image</td></tr><tr><td height=20 background=../images/menu_bar2.gif></td></tr></table></td>
</tr>
<tr><td align=right style=background-color:white>description »</td><td align=right style=background-color:#e0e0e0><INPUT name=desc type=text value="<%=formarr[enumfld["desc"]]%>" size=<%=ctxtwidth%>></td></tr>
<tr><td align=right style=background-color:white>header »</td><td align=right style=background-color:#e0e0e0><TEXTAREA NOWRAP name=header type=text size=<%=ctxtwidth%> rows=3><%=formarr[enumfld["header"]]%></TEXTAREA></td></tr>

<tr><td colspan=4 align=center style=background-color:#e0e0e0><iframe src="ImagePick.asp?dt=<%=Request.QueryString("id").Item%>&idir=<%=rev_typedesc%>&iwidth=500" width="570" height="150"></iframe></td></tr>
...
<%
	function editor()
	{
		...
		else
		{
			...
			{
				var attr = "";
				Response.Write("<"+"script language=\"Javascript\" type=\"text/javascript\" src=\"../includes/tinymce/jscripts/tiny_mce/tiny_mce.js\"><"+"/script>\r\n");

				Response.Write("<"+"textarea name=\"rev\" id=\"rev_rev\" class=\"rev\" "+(attr?attr:"style=width:100%;height:400px;")+">"+formarr[enumfld["rev"]]+"<"+"/textarea>");	
			
				var trailing_js = "//////// TINYMCE ////////\r\n"
						+"tinyMCE.init({\r\n"
						+"        mode : \"exact\",\r\n"
						+"        elements : \"rev_rev\",\r\n"
						+"        theme : \"advanced\",\r\n"
						//+"        editor_selector : \""+richtext_names[format]+"\",\r\n"
						+"        skin : \"o2k7\",\r\n"
