...
<%
	...
	if(act && (todo>=0 || sq==0))
	{
		...
		for(var _j=0;_j<arr.length;_j+=7)
		{
			
			/////////////////////////////////////////////////
			//  P A R S E   E V E R Y   E M A I L   B O D Y

		Response.Write(arr[_j+2]);

		_str_b = arr[_j+4];


		// READ THE AVAILABLE FIELD NAMES
		var _parr = _str_b?_str_b.split("{_"):new Array();	// SEARCH FOR FUNCTION CALLS INSIDE TEMPLATE
			...
			switch(curfield)
			{
				...
				case "EXEC":
						...
						else if(arg["scriptid"])
						{
							...
							var mailparam = arr.slice(_j,_j+7);
							_templdat[_pi] = "<!--EXEC-->";
							eval(script);
						}
				break;
		...
		for(var _pi=0;_pi<_tmplfld.length;_pi++)
			_str_b = _str_b.replace("{_"+_tmplfld[_pi]+"_}",_templdat[_pi]?_templdat[_pi]:("{_"+_tmplfld[_pi]+"_}"));
		...
		else if(act.indexOf("send to")==0)
		{
			for(var _j=0;_j<arr.length;_j+=7)
			{
				...
				if(isValidEmail(arr[_j+2])==true)
				{
					...
					Mail.From 	= arr[_j];
					//Response.Write("*6*<br>");
				 	Mail.FromName   = arr[_j+1]?arr[_j+1]:arr[_j];
					//Response.Write("*7*<br>");
					Mail.Subject 	= arr[_j+3];
					//Response.Write("*8*<br>");
					Mail.Body 	= body?body.replace(/\\"/g,"\""):"";//.replace(/<br>/gi,"\r\n");
					//Response.Write("*9*<br>");
					Mail.IsHTML	= true;
					//Response.Write("*10*<br>");
					Mail.AddAddress(arr[_j+2]);
					...
					if(bDebug)
					{
					  ...
					  Response.Write("Address="+arr[_j+2]+"<br>");
					}
					
					var FSO = Server.CreateObject("Scripting.FileSystemObject");
					
					//Mail.AddAttachment(arr[_j+5])
					//Mail.AddAttachmentMem("test.jpg",Readfile(arr[_j+5]))
					

					// EMBEDDED IMAGES
					if(arr[_j+6])
					{
						var arratt = arr[_j+6].split(",");
					...
					Response.Write("<table cellspacing=1 cellpadding=0 class=small width=500>");
					Response.Write("<tr><td align=right width=50>from:</td><td>"+(arr[_j+1]?(arr[_j+1]+" &lt;"+arr[_j]+"&gt;"):arr[_j])+"</td></tr>");
					Response.Write("<tr><td align=right width=50>to:</td><td>"+arr[_j+2]+"</td></tr>");
					Response.Write("<tr><td align=right width=50>subject:</td><td>"+arr[_j+3]+"</td></tr>");
					//Response.Write("<tr><td align=right width=50>attachments:</td><td>"+arr[_j+5]+"</td></tr>");
					Response.Write("<tr><td align=right width=50>images:</td><td>"+cid_pool.join("<br>")+"</td></tr>");
					Response.Write("</table><hr>");
					Response.Write("<table cellspacing=1 cellpadding=0 class=small width=500>");
					Response.Write("<tr><td colspan=2 style=font-weight:bold>"+(arr[_j+4]?arr[_j+4].replace(/\\"/g,"\""):"")+"</td></tr>");
					Response.Write("</table><hr>");
	...
	<tr>
		<td bgcolor="#D4D0C8">
			<table>
				...
				<tr><td class=small title="Sender name">sender</td><td class=small><input name=sender value="<%=Server.HTMLEncode(Request.QueryString("sender").Item)%>" style=width:300px></td></tr>					
				<tr><td class=small title="E-mail destination address">to</td><td class=small>
				
				<table cellspacing=0 cellpadding=0 align=left><tr><td><small>
				...
				<input type=hidden name=overrideto value="manual">
				
				</small>
				
				<!--br-->
				
				<input type=submit name=act value="set list" style=width:100px>
				
				
				</td></tr></table>
				</tr>		
				
				<tr><td class=small title="E-mail subject">subject</td><td class=small><input name=subject value="<%=Server.HTMLEncode(Request.QueryString("subject").Item)%>" style=width:300px></td></tr>
				...
				<tr><td class=small title="Attachments">embedded images</td><td class=small>	
				<%
						//Response.Write("<table cellspacing=0 cellpadding=0><tr><td><table bgcolor=#FFFFFF cellspacing=10 cellpadding=1 border=0 style=width:300px><tr><td class=small>"+(Session(_pagename+ses+"_3")?Session(_pagename+ses+"_3"):"[empty] &nbsp;")+"</td></tr></table></td><td class=small ></td></tr></table><br>")	
						Response.Write("<br><input type=submit name=act value=\"set embedded images\" style=width:100px>")				
				%>
				</td></tr>				
