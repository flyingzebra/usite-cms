...
<%
	...
	var tablefld = new Array("rev_id","rev_ref","rev_title","rev_desc","rev_header","rev_rev","rev_rt_cat","rev_address","rev_phone","rev_fax","rev_crea_date","rev_mod_acc_id","rev_mod_date","rev_publisher","rev_email","rev_url","rev_date_published","rev_pub" ) 
	var formfld  = new Array("id"    ,"refcat"        ,"title"    ,"desc"    ,"header"    ,"rev"    ,"category"  ,"address"    ,"phone"    ,"fax"    ,"crea_date"    ,"mod_acc"	     ,"mod_date"    ,"publisher"    ,"email"    ,"url"    ,"pub_date"          ,"status"    ,"act", "source")
	...
	bMultipart = false;
	try { var Upload = Server.CreateObject("Persits.Upload.1");  var FormCollection = Upload.Form; bUpload = true; } catch(e) { bMultipart = false; function Upload() { this.Form = UploadForm; } function UploadForm() {} }
	...
	var bSubmitted = bMultipart?(bUpload && (!new Enumerator(FormCollection).atEnd())):(Request.TotalBytes==0?false:true);
...
//Response.End();
	...
	for (var i=1;i<tablefld.length;i++)
	{
		...
		{
			botharr[i-1] = tablefld[i] + "=" + (formarr[i] && typeof(formarr[i])=="string"?("\""+formarr[i].replace(/\\/g,"\\\\").replace(/\x22/g,"\\\"")+"\""):formarr[i]?formarr[i]:"null");
		}
	}
	...
	if (bSubmitted)
	{
		if (bExists==true)
		{
			...
			//Response.Write(Server.HTMLEncode(uSQL))
			...
			{	
			    ...
			    if(!_ws)
				{
                 Response.Write("session expired, please login again<br>");
				}
				else
				{
					var src = Server.MapPath("../"+_ws+"/lib/")+"\\"+formarr[enumfld["title"]];
					var src2 = Server.MapPath("../"+_ws+"/lib")+"\\__"+formarr[enumfld["title"]]
					
					var fso = Server.CreateObject("Scripting.FileSystemObject");
					//Response.Write(fso.FileExists(src)+"<br>");
				
					
					if(fso.FileExists(src))
					{
						if(fso.FileExists(src2)==true)
					      fso.DeleteFile(src2);
					    fso.MoveFile(src,src2);
					   ...
					   var file = fso.CreateTextFile(src,true);
					   file.WriteLine(formarr[enumfld["rev"]]);
					   file.close();
					}
					else
					{
	...
	// HANDLE QUOTES
	//for (var i=0;i<formfld.length;i++)
	//  formarr[i] = formarr[i] && typeof(formarr[i])=="string"?formarr[i].replace(/\x22/g,"\&quot;"):formarr[i];
...
<tr><td align="right" style="background-color:white"><%=_T["admin_category"]%>&nbsp;»</td><td align="right" style="background-color:#e0e0e0"><%=rev_categories%></td><td align=left style=background-color:#e0e0e0></td><td align="left" style="background-color:white">«&nbsp;<%=_T["admin_skin"]%></td></tr>
