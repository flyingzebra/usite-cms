...
<%
	...
	for (j = 0; j<topic.length; j+=cols )
	{
		for(i=j;i<(j+cols);i++)
			if (i<topic.length)
				if (topic[i].substring(0,1)!="*")
					s += "<td align=center width=110 height=110 valign=top>"+sbox("<a href="+menulink[i]+"><img src=../images/"+topic[i]+" height=100 width=100 border=0></a>",100)+"<font face=verdana size=2>"+topicname[i]+"</font></td>";
				else
					s += "<td align=center width=110 height=110 valign=top>"+sbox(topic[i].substring(1,topic[i].length),100)+"<font face=verdana size=2>"+topicname[i]+"</font></td>";
			else
				s += "<td width=110></td>";
