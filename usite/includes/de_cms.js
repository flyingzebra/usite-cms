...
dl = document.layers;da = document.all;ge = document.getElementById;ws = window.sidebar;var msg='';function nem(){return true};window.onerror = nem;var s64;
	function doToolbar() {
		...
		for (b=0; b < allInputs.length; b++) {
			if (borderShown == "no") {
				if (allInputs[b].type.toUpperCase() == "HIDDEN") {
					...
					allInputs[b].runtimeStyle.backgroundImage = "url(" + deveditPath1 + "/images/hidden.gif)"
		...
		for (a=0; a < allLinks.length; a++) {
			if (borderShown == "no") {
				if (allLinks[a].href.toUpperCase() == "") {
					...
					allLinks[a].runtimeStyle.backgroundImage = "url(" + deveditPath1 + "/images/anchor.gif)"
				}
			} else {
